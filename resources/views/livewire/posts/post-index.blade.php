<div>
                                    ...
                                    <tr class="hover:bg-gray-50" wire:key="post-{{ $post->id }}">
                                        ...
                                        <td class="px-6 py-4 whitespace-nowrap text-sm font-medium">
                                            <div class="flex space-x-2">
                                                ...
                                                @if($this->canEdit())
                                                    <button wire:click="confirmStatusChange({{ $post->id }})"
                            ...
                            <div class="mt-3 text-center sm:ml-4 sm:mt-0 sm:text-left">
                                ...
                                <div class="mt-2">
                                    <p class="text-sm text-gray-500">
                                        Are you sure you want to delete the post <strong>"{{ $postToDeleteTitle }}"</strong>? This action cannot be undone.
                                    </p>
                                </div>
                            </div>
                        </div>
    ...
    @if($showStatusModal)
        <div class="fixed inset-0 z-50 overflow-y-auto" 
            ...
            <!-- Background overlay -->
            <div class="fixed inset-0 bg-gray-500 bg-opacity-75 transition-opacity"
                 @click="$wire.closeStatusModal()"></div>
            ...
            <div class="flex min-h-full items-center justify-center p-4 text-center sm:p-0">
                <div class="relative transform overflow-hidden rounded-lg bg-white text-left shadow-xl transition-all sm:my-8 sm:w-full sm:max-w-lg"
                    ...
                    <div class="bg-white px-4 pb-4 pt-5 sm:p-6 sm:pb-4">
                        <div class="sm:flex sm:items-start">
                            <!-- Status icon -->
                            <div class="mx-auto flex h-12 w-12 flex-shrink-0 items-center justify-center rounded-full {{ $newStatus === 'published' ? 'bg-green-100' : 'bg-yellow-100' }} sm:mx-0 sm:h-10 sm:w-10">
                            ...
                            <div class="mt-3 text-center sm:ml-4 sm:mt-0 sm:text-left">
                                <h3 class="text-base font-semibold leading-6 text-gray-900">
                                    {{ ucfirst($newStatus) }} Post
                                </h3>
                                <div class="mt-2">
                                    <p class="text-sm text-gray-500">
                                        Are you sure you want to {{ $newStatus === 'published' ? 'publish' : 'unpublish' }} the post <strong>"{{ $postToChangeStatusTitle }}"</strong>?
                                        @if($newStatus === 'published')
                                            This will make the post visible to the public.
                                        @else
                                            This will hide the post from the public.
                                        @endif
                                    </p>
                                </div>
                            </div>
                    ...
                    <div class="bg-gray-50 px-4 py-3 sm:flex sm:flex-row-reverse sm:px-6">
                        <button type="button" 
                                wire:click="changePostStatus"
                                wire:loading.attr="disabled"
                                wire:target="changePostStatus"
                                class="inline-flex w-full justify-center rounded-md {{ $newStatus === 'published' ? 'bg-green-600 hover:bg-green-500' : 'bg-yellow-600 hover:bg-yellow-500' }} px-3 py-2 text-sm font-semibold text-white shadow-sm disabled:opacity-50 sm:ml-3 sm:w-auto">
                            <span wire:loading.remove wire:target="changePostStatus">
                                {{ ucfirst($newStatus) }}
                            </span>
                            <span wire:loading wire:target="changePostStatus" class="flex items-center">
                                <svg class="animate-spin -ml-1 mr-2 h-4 w-4 text-white" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24">
                                    <circle class="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" stroke-width="4"></circle>
                                    <path class="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4zm2 5.291A7.962 7.962 0 014 12H0c0 3.042 1.135 5.824 3 7.938l3-2.647z"></path>
                                </svg>
